...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{ route('weblinks.update', $model->slug) }}" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				@csrf
				{{ method_field('PATCH') }}
				<div class="box box-info">
					<div class="box-body">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<select name="brand_name" id="brand_name" class="form-control" required>
									<option value="" selected>Select Brand</option> <!-- Default empty option -->
									@foreach ($brands as $brand)
										<option value="{{ $brand->name }}" {{ $model->brand_name == $brand->name ? 'selected' : '' }}>
											{{ $brand->name }}
										</option>
									@endforeach
								</select>
						...
						<div class="form-group">
                            <label for="" class="col-sm-2 control-label">UI Design</label>
                            <div class="col-sm-6" style="padding-top:5px">
                                <input type="file" class="form-control" accept="image*" name="image" id="image" >
                            </div>
                            @if(!empty($model->image))
                                <div class="col-sm-4">
                                    <img style="width: 80px" src="{{ asset('public/admin/assets/images/weblinks') }}/{{ $model->image }}" alt="">
                                </div>
                            @else
                                <div class="col-sm-4" >
