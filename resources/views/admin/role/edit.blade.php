...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{ route('role.update', $role->id) }}" id="regform" class="form-horizontal" enctype="multipart/form-data" method="post" accept-charset="utf-8">
				@csrf
				{{ method_field('PATCH') }}
						...
						<div class="form-group">
							...
							<div class="col-sm-9">
								@foreach($permission as $value)
									<div class="col-sm-3">
										<label>
											<input type="checkbox" name="permission[]" value="{{ $value->id }}" class="name" {{ in_array($value->id, $rolePermissions) ? 'checked' : '' }}> {{ ucfirst($value->name) }}
										</label>
									</div>
								@endforeach
							</div>
